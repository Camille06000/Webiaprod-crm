...
const seeds = [
  {
    ...
    raison_fit: "Top 10 local mais invisible ChatGPT", accroche: "Vos concurrents apparaissent dans ChatGPT, pas vous.",
    objet_email: "Lyon · ChatGPT envoie ses clients chez vos concurrents", gmail_draft_id: "DRAFT-001",
    status: "nouveaux",
  },
  {
  ...
  },
  {
    ...
    note_google: 4.8, nb_avis: 1245,
    top10_google: 1, visible_chatgpt: 1,
    email_pro_verifiable: 1, site_actif: 1, description_claire: 1, gbp_bien_rempli: 1,
  ...
  },
  {
    ...
    email: "contact@autoecole-bordeaux.fr",
    site: "https://autoecole-bordeaux.fr",
    note_google: 3.9, nb_avis: 12,
    top10_google: 0, visible_chatgpt: 0,
  ...
  },
  {
    ...
    email: "secretariat@dentaire-lille.fr",
    site: "https://dentaire-lille.fr",
    note_google: 4.9, nb_avis: 156,
    top10_google: 1, visible_chatgpt: 0,
    email_pro_verifiable: 1, site_actif: 1, description_claire: 1, gbp_bien_rempli: 1,
    status: "nouveaux",
  },
...
const cols = [
  ...
  "note_google","nb_avis","top10_google","visible_chatgpt",
  "email_pro_verifiable","site_actif","description_claire","gbp_bien_rempli",
...
const insert = db.prepare(`INSERT INTO leads (${cols.join(",")}) VALUES (${cols.map(() => "?").join(",")})`);
...
for (const s of seeds) {
  ...
  const vals = cols.map((c) => {
    ...
    if (c === "date_prochaine_action") return s.date_prochaine_action ?? next.date;
    if (c === "prochaine_action_type") return s.prochaine_action_type ?? next.type;
    return s[c] ?? null;
  });
  insert.run(...vals);
}

console.log(`Seed ok: ${seeds.length} leads inserés.`);
